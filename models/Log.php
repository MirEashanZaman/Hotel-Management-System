...
class Log extends BaseModel {
    public function getLogs($limit = 100) {
        $limit = intval($limit);
        $result = $this->db->query("SELECT l.*, u.name as user_name, u.role FROM activity_logs l LEFT JOIN users u ON l.user_id = u.id ORDER BY l.logged_at DESC LIMIT $limit");
